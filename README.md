📊 Andes Retail Group: Dashboard de Desempeño Comercial

📌 Descripción del Proyecto
Diseño y desarrollo de un dashboard ejecutivo interactivo para analizar el desempeño comercial de Andes Retail Group en Perú, Chile y Colombia durante 2024–2025.
El proyecto transformó datos transaccionales en información estratégica para evaluar ingresos, rentabilidad, segmentos de clientes, categorías de producto y tendencias temporales, facilitando la toma de decisiones comerciales basada en datos.

🎯 Objetivos
Analizar la evolución comercial entre 2024 y 2025.
Identificar segmentos de clientes con mayor valor para el negocio.
Evaluar el desempeño de categorías de producto.
Comparar resultados entre países y regiones.
Detectar patrones estacionales y oportunidades de crecimiento.
Comunicar hallazgos mediante dashboards ejecutivos orientados a negocio.

🔍 Metodología
1. Preparación y Validación de Datos
Revisión de calidad de datos.
Validación de tipos de datos y consistencia.
Creación de métricas derivadas para análisis comercial.
2. Diseño Analítico
Definición de KPIs clave.
Estructuración del dashboard bajo principios de diseño visual profesional.
Construcción de narrativa basada en el modelo SQCA.
3. Dashboard Ejecutivo

Desarrollo de dos vistas principales:
Vista General (Overview)
Ingresos Totales
Ganancia Total
Unidades Vendidas
Evolución temporal
Comparativo por país
Desempeño por segmento

Vista Detallada
Categorías de producto
Segmentos de clientes
Regiones
Análisis exploratorio mediante filtros interactivos
Detección de oportunidades comerciales

📈 Principales Resultados
✅ Identificación de categorías con mayor contribución al ingreso y rentabilidad.
✅ Comparación del desempeño comercial entre Perú, Chile y Colombia.
✅ Detección de segmentos de clientes con mayor valor para el negocio.
✅ Análisis temporal para identificar patrones estacionales y tendencias de crecimiento.
✅ Desarrollo de dashboard interactivo para monitoreo ejecutivo y análisis comercial.

💡 Insights de Negocio
Algunas categorías concentraron una proporción significativa de los ingresos totales.
Existen diferencias relevantes de desempeño entre regiones y países.
Determinados segmentos de clientes aportan mayor rentabilidad al negocio.
Los patrones temporales permiten identificar temporadas con mayor actividad comercial.
El análisis facilita priorizar acciones de crecimiento y optimización comercial.
🛠️ Herramientas Utilizadas
Power BI
Tableau
Power Query
Data Modeling
KPI Analysis
Dashboard Design
Data Storytelling
SQCA Framework
Business Intelligence

📁 Estructura del Proyecto
dashboard/
│
├── AndesRetail_Dashboard.pbix
│
data/
│
├── Andes_Retail_Group_2024_2025.xlsx
│
documentation/
│
├── business_requirements.md
├── dashboard_storytelling.md
│
reports/
│
├── executive_summary.pdf

🎓 Contexto Académico
Proyecto 9 – TripleTen Data Analyst Bootcamp (2026)
Proyecto enfocado en Business Intelligence y visualización de datos, orientado a transformar información transaccional en dashboards ejecutivos que permitan identificar oportunidades comerciales y apoyar la toma de decisiones estratégicas.

🚀 Competencias Demostradas
Business Intelligence
Commercial Analytics
KPI Monitoring
Executive Reporting
Dashboard Development
Data Storytelling
Data Visualization
Power BI
Tableau
Customer Segmentation
Trend Analysis
Business Communication
